import React from 'react'

const AcceptTask = ({ data }) => {
    return (
        <div className='snap-start flex-shrink-0 min-w-[280px] w-[84vw] sm:w-[300px] bg-gray-400 rounded-2xl p-5 font-medium shadow-md'>
            <div className='flex justify-between items-center'>
                <h3 className='bg-emerald-950 text-sm md:text-lg px-3 py-1 rounded text-white'>{data.category}</h3>
                <h4 className='text-sm text-slate-800 dark:text-slate-200'>{data.taskDate}</h4>
            </div>
            <h2 className='mt-4 text-xl md:text-2xl font-semibold text-slate-900 dark:text-slate-100'>{data.taskDescription}</h2>
            <p className='text-sm mt-2 text-slate-700 dark:text-slate-300'>{data.taskTitle}</p>
            <div className='flex flex-col sm:flex-row gap-3 mt-5'>
                <button className='flex-1 bg-green-600 py-3 text-sm rounded-lg text-white active:scale-[0.99]'>Mark As Completed</button>
                <button className='flex-1 bg-red-500 py-3 text-sm rounded-lg text-white active:scale-[0.99]'>Mark As Failed</button>
            </div>
        </div>
    )
}

export default AcceptTask